<template>
<v-container class="mb-15 disable-dbl-tap-zoom">
    <v-col>
    <v-row v-bind:key="wordlist" v-for="(wordlist, i) in words" class="ml-n12">
        <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        getRandomlist() {
            var arr = this.randomArray(this.wordArray, 25)
            var outputList = []
            for(var i = 0; i < 5; i++) {
                outputList.push(arr.slice(i*5, i*5+5))
            }
